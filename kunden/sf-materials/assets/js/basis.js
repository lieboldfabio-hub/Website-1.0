...
window.Basis = (function () {
  ...
  if (burger && menu) {
    ...
    function setzeMenu(neu) {
      ...
      burger.setAttribute("aria-label", offen ? "Close menu" : "Open menu");
      document.body.style.overflow = offen ? "hidden" : "";
      /* Der Fokus kann erst in das Menue wandern, wenn es sichtbar ist.
         Waehrend des Uebergangs gilt es dem Browser noch als unsichtbar und
         nimmt keinen Fokus an, deshalb erst am Ende des Uebergangs. Der
         Zeitgeber ist der Rueckfall, falls gar kein Uebergang laeuft. */
      if (offen) {
        var inMenue = function () {
          if (offen) menu.querySelector("a").focus();
        };
        menu.addEventListener("transitionend", inMenue, { once: true });
        setTimeout(inMenue, 420);
      }
    }
